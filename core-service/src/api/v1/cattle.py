...
# NO IMPORTAR Session aquí, ya está en get_db
...
async def create_cattle(
    ...
    current_user: UserAuth = Depends(get_current_user),  # ✅ Auth primero
    db = Depends(get_db),  # ✅ Sin tipo explícito para evitar error
):
...
async def get_cattle_list(
    ...
    """Listar todo el ganado del usuario"""
    service = CattleService(db)
    result = service.get_user_cattle(
        owner_id=current_user.id,
...
async def search_cattle(
    ...
    service = CattleService(db)
    cattle = service.search_by_lote(query, current_user.id)
    return cattle
...
async def get_cattle(
    ...
    service = CattleService(db)
    cattle = service.get_cattle(cattle_id, current_user.id)
...
async def delete_cattle(
    ...
    """Eliminar animal"""
    service = CattleService(db)
    deleted = service.delete_cattle(cattle_id, current_user.id)
